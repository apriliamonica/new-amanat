...
const { isAdmin, isKetua, canValidate } = require('../middleware/rbac');
...
// Admin only routes
router.post('/', isAdmin, upload.single('file'), suratKeluarController.createSuratKeluar);
router.put('/:id', isAdmin, upload.single('file'), suratKeluarController.updateSuratKeluar);
router.delete('/:id', isAdmin, suratKeluarController.deleteSuratKeluar);

// Validate (Sekpeng or Bendahara)
router.put('/:id/validasi', canValidate, suratKeluarController.validateSuratKeluar);
